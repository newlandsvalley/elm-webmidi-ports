...
update msg model =
    case msg of
            ...
            case midiMsg of
                OutputDeviceDisconnected dev ->
                    let
                        ...
                        newId =
                            if Just dev.id == model.maybeId then
                                List.head (Dict.keys newWebMidi.outputDevices)
                            else
                                model.maybeId

                        newModel =
                            { model | webMidi = newWebMidi, maybeId = newId }
                    in
        ...
        ChangeId newId ->
            { model | maybeId = Just (log "newId" newId) } ! []
